...
const menuItems = [
  ...
  { href: '/admin/kyc-applications', label: 'KYC Applications', icon: ShieldCheck },
...
export function AdminSidebar() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from('.admin-sidebar-menu-item', {
        opacity: 0,
        x: -20,
        duration: 0.6,
        stagger: 0.1,
      ...
      gsap.from('.admin-sidebar-footer-item', {
        opacity: 0,
        y: 20,
        duration: 0.6,
        stagger: 0.1,
  ...
  return (
    <Sidebar ref={sidebarRef}>
      <SidebarHeader className="border-b border-white/10 p-4">
        <Link href="/admin/dashboard" className="admin-sidebar-logo group relative flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-1">
              ...
              <Sparkles className="admin-sidebar-sparkle h-3 w-3 text-emerald-400" />
            </div>
      ...
      <SidebarContent className="p-2">
        <SidebarMenu>
          {menuItems.map((item) => (
            <SidebarMenuItem key={item.label} className="admin-sidebar-menu-item">
      ...
      <SidebarFooter className="border-t border-white/10 p-2">
        <SidebarMenu>
          {/* Settings */}
          <SidebarMenuItem className="admin-sidebar-footer-item">
            <Link href="/admin/settings" className="block">
              ...
              >
                ...
                <Settings className={`transition-transform duration-300 group-hover:rotate-90 ${
                  isActive('/admin/settings') ? 'text-white' : ''
                }`} />
